...
> **When to use:** Use to set up a full customer-support agent for a web app — runs the whole build pipeline (knowledge base, read-only tools + access SQL, diagnostic runbooks, persona, config/secrets scaffolding) and reports everything needing review. Invoke when the user says "set up a support agent for <path>", "stand up the support kit", or "build the full support pipeline for <app>".
